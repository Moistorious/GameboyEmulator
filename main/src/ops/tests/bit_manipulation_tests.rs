...
fn test_bit_all_registers() {
    ...
    for &reg in regs.iter() {
        ...
        gb.bit(bit_opcode(0, reg)).unwrap();
        ...
        gb2.bit(bit_opcode(0, reg)).unwrap();
...
fn test_bit_all_bits() {
    for bit in 0..8u8 {
        let mut gb = Gameboy::new();
        gb.cpu.b = 1 << bit;
        gb.bit(bit_opcode(bit, Reg8::B)).unwrap();
        assert!(gb.cpu.f & Gbz80::FLAG_Z == 0, "BIT {},B (set) should clear Z", bit);

        let mut gb2 = Gameboy::new();
        gb2.cpu.b = !(1u8 << bit);
        gb2.bit(bit_opcode(bit, Reg8::B)).unwrap();
...
fn test_bit_preserves_carry() {
    ...
    gb.bit(bit_opcode(0, Reg8::B)).unwrap();
...
fn test_set_all_registers() {
    ...
    for &reg in regs.iter() {
        let mut gb = Gameboy::new();
        gb.cpu.write_reg8(reg, 0x00);
        gb.set(set_opcode(3, reg)).unwrap(); // SET 3, r
...
fn test_set_all_bits() {
    for bit in 0..8u8 {
        let mut gb = Gameboy::new();
        gb.cpu.b = 0x00;
        gb.set(set_opcode(bit, Reg8::B)).unwrap();
...
fn test_set_does_not_modify_flags() {
    ...
    gb.cpu.b = 0x00;
    gb.cpu.f = 0xF0;
    gb.set(set_opcode(0, Reg8::B)).unwrap();
...
fn test_res_all_registers() {
    ...
    for &reg in regs.iter() {
        ...
        gb.res(res_opcode(3, reg)).unwrap(); // RES 3, r
...
fn test_res_all_bits() {
    for bit in 0..8u8 {
        let mut gb = Gameboy::new();
        gb.cpu.b = 0xFF;
        gb.res(res_opcode(bit, Reg8::B)).unwrap();
...
fn test_res_does_not_modify_flags() {
    ...
    gb.cpu.b = 0xFF;
    gb.cpu.f = 0xF0;
    gb.res(res_opcode(0, Reg8::B)).unwrap();
