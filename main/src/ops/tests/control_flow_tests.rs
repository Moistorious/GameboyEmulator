...
fn test_jp_cc_nn_not_taken() {
    // NZ not taken when Z set; PC advances past the 2-byte operand
    let mut gb = Gameboy::new();
    gb.cpu.program_counter = 0x100;
    gb.memory.write_u16(0x100, 0x1234);
...
fn test_jr_n() {
    ...
    gb.jr(0x18);
    assert_eq!(gb.cpu.program_counter, 0x106);
}
...
fn test_jr_n_negative() {
    let mut gb = Gameboy::new();
    gb.cpu.program_counter = 0x100;
    gb.memory.write_u8(0x100, 0xFB); // -5
    gb.jr(0x18);
    assert_eq!(gb.cpu.program_counter, 0x0FC);
}
...
fn test_call_nn() {
    ...
    assert_eq!(gb.memory.read_u16(0xFFFC), 0x102);
}
...
fn test_ret_cc_taken_and_not() {
    ...
    // RET Z not taken: PC and SP unchanged
    let mut gb2 = Gameboy::new();
